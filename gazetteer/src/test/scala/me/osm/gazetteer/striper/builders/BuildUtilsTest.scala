...
class BuildUtilsTest extends FlatSpec with Matchers {
    ...
    def loadWaysFromText(txt: String): Iterable[LineString] = {
        import Toolbox.StringOps

        def buildCoordinate(xy: Seq[String]): Coordinate = {
            new Coordinate(xy(0).toDouble, xy(1).toDouble)
        }

        def loadLineString(str: String): LineString = {
    ...
    def ringEqual(left: LineString, right: String): Boolean = {
